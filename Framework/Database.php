...
class Database {
	...
	public function connect() {
		try {
			if(empty($this->connection->dsn) || empty($this->connection->user) || empty($this->connection->password)) {
				throw new Error(['class'=>__CLASS__,'method'=>__METHOD__,'line'=>__LINE__,'file'=>__FILE__,'severity'=>ERROR_FATAL,'message'=>"No valid database connection string",'description'=>"<p>Please check your configuration file.</p>"]);
			} else {
				try {
					...
					throw new Error(['class'=>__CLASS__,'method'=>__METHOD__,'line'=>__LINE__,'file'=>__FILE__,'severity'=>ERROR_FATAL,'message'=>"Cannot connect to database with DSN '{$this->connection->dsn}'",'description'=>"<p>Please check your configuration file.</p>"]);
				}
			}
		} catch(Error $_error) {
	...
	public function load($query = null,$list = null) {
		...
		$sth->execute($list);
		$result = $sth->fetchAll(\PDO::FETCH_CLASS|\PDO::FETCH_PROPS_LATE,'\\'.__CUBO__.'\\Model\\'.$this->className);
		$this->className = null;
		return $result;
	}
	...
	public function loadObject($query = null,$list = null) {
		...
		$sth = $this->dbh->prepare(empty($query) ? $this->query() : $query);
		$sth->execute($list);
		$sth->setFetchMode(\PDO::FETCH_CLASS|\PDO::FETCH_PROPS_LATE,'\\'.__CUBO__.'\\Model\\'.$this->className);
		$result = $sth->fetch();
		$this->className = null;
		return $result;
	}
